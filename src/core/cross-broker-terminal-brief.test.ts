...
test("bangtong compact Terminal Brief emitted on every terminal status: failed", () => {
  ...
  assert.equal(event.payload.terminalBriefTitle, "A2A Terminal Brief 실패: bangtong(0/7)");
  assert.equal(event.payload.status, "failed");
  assert.equal(event.payload.parentRoundProgress, 0);
});
...
test("bangtong compact Terminal Brief emitted on every terminal status: canceled", () => {
  ...
  assert.equal(event.payload.terminalBriefTitle, "A2A Terminal Brief 취소: bangtong(0/7)");
...
test("bangtong compact Terminal Brief emitted on every terminal status: blocked", () => {
  ...
  assert.equal(event.payload.terminalBriefTitle, "A2A Terminal Brief 차단: bangtong(0/7)");
...
test("cross-broker duplicate projection is idempotent; terminal event title unchanged", () => {
  ...
  assert.equal(matching.length, 1, "duplicate projection must not create a second terminal event");

  const event = matching[0];
  assert.ok(event);
  assert.equal(event.payload.terminalBriefTitle, "A2A Terminal Brief 완료: bangtong(1/5)");
  assert.equal(event.payload.status, "succeeded");
  assert.equal(event.payload.parentRoundProgress, 1);
  assert.equal(event.payload.parentRoundTotal, 5);
});

test("cross-broker mixed succeeded and failed projections: numerator counts only succeeded", () => {
  const broker = new InMemoryA2ABroker(undefined, undefined, { brokerId: "parent-broker" });
  createParentRound(broker);
  ...
  // Failed projection: succeeded count is 0 (no tasks have succeeded yet)
  ...
  // Succeeded projection: succeeded count is 1
