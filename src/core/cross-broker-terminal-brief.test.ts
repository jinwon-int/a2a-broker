...
test("cross-broker Terminal Brief projection carries parent round denominator into outbox progress", () => {
  ...
  assert.equal(terminalEvents[0]?.payload.parentRoundProgress, 1); // canonical: 1st succeeded event in round (a2a-broker#656)
...
test("Seoseo-origin parent keeps distinct Gwakga handoff children and explicit order", () => {
  ...
  assert.equal(terminalEvents[1]?.payload.parentRoundProgress, 2); // canonical: 2nd succeeded event in round (a2a-broker#656)
...
test("cross-broker Terminal Brief projection is symmetric for Gwakga-owned parent rounds", () => {
  ...
  assert.equal(terminalEvent?.payload.parentRoundProgress, 1); // canonical: 1st succeeded event in round (a2a-broker#656)
...
test("cross-broker Terminal Brief projections survive broker snapshot persistence", () => {
  ...
  // Verify canonical progress survives snapshot restore (a2a-broker#656)
  const terminalEvents = restored.getTerminalTaskEventOutbox().subscribe();
  assert.equal(terminalEvents.length, 1);
  assert.equal(terminalEvents[0]?.payload.parentRoundProgress, 1);
  assert.equal(terminalEvents[0]?.payload.parentRoundTotal, 5);
});
...
test("bangtong compact Terminal Brief emitted on every terminal status: blocked", () => {
  ...
  // Blocked tasks do not count toward canonical completed progress (a2a-broker#656)
  assert.equal(event.payload.parentRoundProgress, 0);
  assert.equal(event.payload.terminalBriefTitle, undefined);
